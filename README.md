...
## 「Can't set compatibility date in the future」というデプロイエラーが出たら

`wrangler.jsonc` の `compatibility_date` に設定した日付が、Cloudflareのビルド
サーバーの時計から見て「まだ来ていない日」だと判定されると出るエラーです
（UTC基準のため、日本時間の「今日」がビルドサーバー側ではまだ前日、という
ことがあります）。日付を「今日」に合わせず、確実に過去になっている日付
（例: 実際にビルドする日より1〜2週間前）にしておけば起きません。このプロジェクト
では `2026-08-01` のように固定してあります。基本的にこの日付は一度設定したら
触らなくて大丈夫です（Workerの新しい機能を使いたくなった時だけ、その時点の
過去日付に更新してください）。

## ローカルで `git init` した場合のブランチ名について

環境によっては `git init` のデフォルトブランチ名が `master` になります。
Cloudflare側は基本的に `main` ブランチを本番用として扱うので、`master` の
ままだと取得に失敗することがあります。次のいずれかで揃えてください。

```bash
git branch -M main
```

または、Cloudflareのプロジェクト設定（Settings → Builds → Branch control）
で、本番ブランチを `master` 側に合わせて変更することもできます。
